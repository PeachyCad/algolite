...
void error(string text_error) {
	cout << ERROR_COLORIZED_COUT << "Строка " << prev_symbol_line << ", символ " << prev_symbol_pos << ": ";
...
vector<unique_ptr<ArgumentNode>> functionArguments() {
	...
	while (true) {
		...
		while (symbol == LBracket) {
			...
			if (symbol == RBracket) {
				if (bracket_flag)
					error("[] должно быть указано один раз первым по порядку");
				else {
...
Program program() {
	...
	if (!has_main)
		error("Отсутствует начальная точка для выполнения программы в виде описанной функции main");
...
int main() {
	...
	cout << SUCCESS_COLORIZED_COUT << "Транспиляция прошла успешно" << STOP_COLORIZED_COUT << "\n\n";
